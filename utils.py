...
Fourier = {}
def FDA_source_to_target_np( src_img_np, trg_img_np, L=0.1 ):
    # exchange magnitude
    # input: src_img, trg_img
    # get fft of both source and target
    
    key_src = str(src_img_np)
    if key_src not in Fourier:
        fft_src_np = np.fft.fft2( src_img_np, axes=(-2, -1) )
        amp_src, pha_src = np.abs(fft_src_np), np.angle(fft_src_np)
        pha_src_exp= np.exp( 1j * pha_src )
        Fourier[key_src] = (amp_src, pha_src_exp )
    else:
        amp_src, pha_src_exp = Fourier[key_src]

    key_trg = str(trg_img_np)
    if key_trg not in Fourier:		
        fft_trg_np = np.fft.fft2( trg_img_np, axes=(-2, -1) )
        amp_trg, _= np.abs(fft_trg_np), np.angle(fft_trg_np)
        Fourier[key_trg] = amp_trg
    else:
        amp_trg = Fourier[key_trg]

    # mutate the amplitude part of source with target
    amp_src_ = low_freq_mutate_np( amp_src, amp_trg, L=L )

    # mutated fft of source
    fft_src_ = amp_src_ * pha_src_exp

...
from cityscapesSSL import CityScapesSSL
import torch
...
def train(args, model,model_D1, optimizer, optimizer_D1, dataloader_train, dataloader_target,dataloader_targetSSL, dataloader_val):
    ...
    for epoch in range(args.num_epochs):
        ...
        for (data, label, _),(dataSSL, labelSSL, _), (data_t, _, _) in zip(dataloader_train,dataloader_targetSSL, dataloader_target):
            ...
            data_t = data_t.cuda()
            dataSSL= dataSSL.cuda()
            labelSSL = labelSSL.long().cuda()
...
def parse_args():
    ...
    parse.add_argument('--path_model',
                      type=str,
                      default='./results-trialFDA-beta0.01/best_FDA_0.01.pth',
    )
    parse.add_argument('--path_discriminator',
                      type=str,
                      default='./results-trialFDA-beta0.01/best__D1_FDA_0.01.pth',
    )
...
def main():
    ...
    target_datasetSSL = CityScapesSSL(mode="train", max_iter=len(train_dataset))
